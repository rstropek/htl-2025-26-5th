...
try
{
    ...
    var importer = host.Services.GetRequiredService<ISplitImporter>();

    // Perform the import
    var importedCount = await importer.ImportFromCsvAsync(parsedArgs.CsvFilePath, parsedArgs.LaufbewerbId, parsedArgs.IsDryRun);

    Console.WriteLine($"\nSuccessfully imported {importedCount} runner(s).");
...
static void ConfigureServices(IServiceCollection services, IConfiguration configuration)
{
    ...
    services.AddScoped<ISplitCsvParser, SplitCsvParser>();
    services.AddScoped<ISplitDatabaseWriter, SplitDatabaseWriter>();
    services.AddScoped<ISplitImporter, SplitImporter>();
}
